...
import 'package:mobile_scanner/src/enums/android_camera_lens_mode.dart';
...
class MobileScannerController extends ValueNotifier<MobileScannerState> {
  ...
  MobileScannerController({
    ...
    this.useNewCameraSelector = false,
        ...
        _androidCameraLensMode = androidCameraLensMode,
  ...
  AndroidCameraLensMode _androidCameraLensMode;

  /// The preferred Android rear camera lens.
  ///
  /// Non-Android platforms ignore this value.
  AndroidCameraLensMode get androidCameraLensMode => _androidCameraLensMode;
  ...
  /// Update the preferred Android rear camera lens.
  ///
  /// When the scanner is already running, the active camera is restarted so
  /// the new physical rear lens can be selected.
  Future<void> setAndroidCameraLensMode(
    AndroidCameraLensMode lensMode,
  ) async {
    if (_androidCameraLensMode == lensMode) {
      return;
    }

    _androidCameraLensMode = lensMode;

    if (!value.isInitialized || !value.isRunning) {
      return;
    }

    final CameraFacing currentDirection = value.cameraDirection;
    await stop();
    await start(cameraDirection: currentDirection);
  }
